...
void TraCIScenarioManager::init_traci() {
...

    {
        ...
        uint8_t variableNumber = 1;
        uint8_t variable1 = ID_LIST;
        TraCIBuffer buf = queryTraCI(CMD_SUBSCRIBE_PERSON_VARIABLE, TraCIBuffer() << beginTime << endTime << objectId << variableNumber << variable1);
        processSubcriptionResult(buf);
        ASSERT(buf.eof());
    }
...
void TraCIScenarioManager::commandSetSpeed(int objectType, std::string nodeId, double speed) {
    ...
    if (objectType == TraCIScenarioManager::VEHICLE_TYPE) {
        ...
        TraCIBuffer buf = queryTraCI(CMD_SET_PERSON_VARIABLE, TraCIBuffer() << variableId << nodeId << variableType << speed);
        ASSERT(buf.eof());
    }
}
...
void TraCIScenarioManager::subscribeToPersonVariables(std::string personId) {
    ...
    TraCIBuffer buf = queryTraCI(CMD_SUBSCRIBE_PERSON_VARIABLE, TraCIBuffer() << beginTime << endTime << objectId << variableNumber << variable1 << variable2 << variable3 << variable4);
    processSubcriptionResult(buf);
    ASSERT(buf.eof());
}
//**************************************************
...
void TraCIScenarioManager::unsubscribeFromPersonVariables(std::string personId) {
    ...
    TraCIBuffer buf = queryTraCI(CMD_SUBSCRIBE_PERSON_VARIABLE, TraCIBuffer() << beginTime << endTime << objectId << variableNumber);
    ASSERT(buf.eof());
}
//**************************************************
...
void TraCIScenarioManager::processVehicleSubscription(std::string objectId, TraCIBuffer& buf) {
    ...
    if (!mod) {
        ...
        addModule(VEHICLE_TYPE, objectId, moduleType, moduleName, moduleDisplayString, p, edge, speed, angle);
...
void TraCIScenarioManager::processPersonSubscription(std::string objectId, TraCIBuffer& buf) {
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j) {
        ...
        if (isokay != RTYPE_OK) {
            ...
            std::set<std::string> walkingPersons;
            for (uint32_t i = 0; i < count; ++i) {
                std::string idstring; buf >> idstring;
                walkingPersons.insert(idstring);
            }

            // check for persons that need subscribing to
            std::set<std::string> needSubscribe;
            std::set_difference(walkingPersons.begin(), walkingPersons.end(), subscribedPersons.begin(), subscribedPersons.end(), std::inserter(needSubscribe, needSubscribe.begin()));
            for (std::set<std::string>::const_iterator i = needSubscribe.begin(); i != needSubscribe.end(); ++i) {
            ...
            std::set<std::string> needUnsubscribe;
            std::set_difference(subscribedPersons.begin(), subscribedPersons.end(), walkingPersons.begin(), walkingPersons.end(), std::inserter(needUnsubscribe, needUnsubscribe.begin()));
            for (std::set<std::string>::const_iterator i = needUnsubscribe.begin(); i != needUnsubscribe.end(); ++i) {
    ...
    if (!mod) {
        // no such module - need to create
        addModule(PERSON_TYPE, objectId, moduleType, moduleName, moduleDisplayString, p, edge, speed, angle);
...
void TraCIScenarioManager::processSubcriptionResult(TraCIBuffer& buf) {
    ...
    else if (commandId_resp == RESPONSE_SUBSCRIBE_PERSON_VARIABLE) processPersonSubscription(objectId_resp, buf);
    else {
