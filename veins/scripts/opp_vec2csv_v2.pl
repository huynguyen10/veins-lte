...
foreach my $fileName (@fileNames) {
	...
	while (<$FILE>) {
		...
		if (m{
			...
			$events{$event."|".$nod_name."|".$COUNT}{"__TIME"} = $time;
			$events{$event."|".$nod_name."|".$COUNT}{"__EVENT"} = $event;
			$events{$event."|".$nod_name."|".$COUNT}{"__NODE"} = $nod_name;
			$events{$event."|".$nod_name."|".$COUNT}{$vec_name} = $value;
	...
	foreach my $eventAndNode (sort { $a cmp $b } keys %events) {
		...
		foreach my $vectorName (sort keys %vectorNames) {
			...
			my $value = $events{$eventAndNode}{$vectorName};
			print "\t".(defined $value ? $value : "");
		}
		print "\n";
	}
