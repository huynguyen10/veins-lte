...
TraCIScenarioManager::~TraCIScenarioManager() {
    ...
    //************* Quang-Huy Nguyen *******************
    cancelAndDelete(myAddPersonTimer);
    //**************************************************
    delete commandIfc;
    delete connection;
}
...
void TraCIScenarioManager::initialize(int stage) {
    ...
    mobRngVeh = getRNG(vehicleRngIndex);
    ...
    numPersons = par("numPersons").longValue();
    mobRngPer = getRNG(personRngIndex);

    myAddPersonTimer = new cMessage("myAddPersonTimer");
    //**************************************************
    ...
    noMorePersons = false;
    noMoreExpectedVehicle = false;
    noMoreActiveVehicles = false;
...
void TraCIScenarioManager::init_traci() {
    {
        ...
        if ((apiVersion >= 10) && (apiVersion <= 15)) {
...

    {
        ...
        if ((connection->traci2omnet(netbounds2).x > world->getPgs()->x) || (connection->traci2omnet(netbounds1).y > world->getPgs()->y)) {
            MYDEBUG << "WARNING: Playground size (" << world->getPgs()->x << ", " << world->getPgs()->y << ") might be too small for vehicle/person at network bounds (" << connection->traci2omnet(netbounds2).x << ", " << connection->traci2omnet(netbounds1).y << ")" << endl;
        }
    }

    {
        ...
        uint8_t variable8 = VAR_MIN_EXPECTED_VEHICLES;
        ...
        processSubcriptionResult(buf, true);
        ASSERT(buf.eof());
    }

    //************* Quang-Huy Nguyen *******************
    {
        // subscribe to number of persons
        uint32_t beginTime = 0;
        uint32_t endTime = 0x7FFFFFFF;
        std::string objectId = "";
        uint8_t variableNumber = 1;
        uint8_t variable1 = ID_COUNT;
...
void TraCIScenarioManager::handleSelfMsg(cMessage *msg) {
    ...
    if (msg == executeOneTimestepTrigger) {
        if (simTime() > 1) {
            ...
            if (edgeIds.size() == 0) {
                std::list<std::string> edges = getCommandInterface()->getEdgeIds();
                for (std::list<std::string>::const_iterator i = edges.begin(); i != edges.end(); ++i) {
                    std::string edgeId = *i;
                    MYDEBUG << "Adding " << edgeId << " to list of possible edges" << std::endl;
...
void TraCIScenarioManager::addModule(int objectType, std::string nodeId, std::string type, std::string name, std::string displayString, const Coord& position, std::string road_id, double speed, double angle) {

    if (hosts.find(nodeId) != hosts.end()) error("tried adding duplicate module");

    //************* Quang-Huy Nguyen *******************
    if (objectType == PERSON_TYPE) {
    ...
    for (cModule::SubmoduleIterator iter(mod); !iter.end(); iter++) {
        ...
        ifInetTraCIMobilityCallPreInitialize(objectType, submod, nodeId, position, road_id, speed, angle);
        if (objectType == PERSON_TYPE) {
            TraCIPersonMobility* mm = dynamic_cast<TraCIPersonMobility*>(submod);
            if (!mm) continue;
            mm->preInitialize(nodeId, position, road_id, speed, angle);
        } else {
            TraCIVehicleMobility* mm = dynamic_cast<TraCIVehicleMobility*>(submod);
            if (!mm) continue;
            mm->preInitialize(nodeId, position, road_id, speed, angle);
        }
    }
    ...
    for (cModule::SubmoduleIterator iter(mod); !iter.end(); iter++) {
        cModule* submod = SUBMODULE_ITERATOR_TO_MODULE(iter);
        ifInetTraCIMobilityCallNextPosition(objectType, submod, position, road_id, speed, angle);
        if (objectType == PERSON_TYPE) {
            TraCIPersonMobility* mm = dynamic_cast<TraCIPersonMobility*>(submod);
            if (!mm) continue;
            mm->changePosition();
...
void TraCIScenarioManager::deleteManagedModule(std::string nodeId) {
    ...
    if (!mod) error("no vehicle/person with Id \"%s\" found", nodeId.c_str());
...
void TraCIScenarioManager::executeOneTimestep() {
    ...
    if (targetTime > round(connectAt.dbl() * 1000)) {
        ...
        insertPersons();
        //**************************************************
        TraCIBuffer buf = connection->query(CMD_SIMSTEP, TraCIBuffer() << targetTime);

        uint32_t count; buf >> count;
        MYDEBUG << "Getting " << count << " subscription results" << endl;
        for (uint32_t i = 0; i < count; ++i) {
            processSubcriptionResult(buf, false);
        }
    }

    if (!noMoreActiveVehicles || !noMoreExpectedVehicle || !noMorePersons) {
...
void TraCIScenarioManager::insertNewPerson() {
    ...
    if (personTypeIds.size()) {
        int perTypeId = mobRngPer->intRand(personTypeIds.size());
        type = personTypeIds[perTypeId];
    }
    else {
        type = "DEFAULT_PEDTYPE";
    }
    int edgeId = mobRngPer->intRand(edgeIds.size());
    personInsertQueue[edgeId].push(type);
}

void TraCIScenarioManager::insertPersons() {

    for (std::map<int, std::queue<std::string> >::iterator i = personInsertQueue.begin(); i != personInsertQueue.end(); ) {
        std::string edge = edgeIds[i->first];
        MYDEBUG << "process " << edge << std::endl;
        std::queue<std::string> persons = i->second;
        while (!i->second.empty()) {
            bool suc = false;
            std::string type = i->second.front();
            std::stringstream per;
            per << type << "_" << personNameCounter;
            MYDEBUG << "trying to add " << per.str() << " with " << edge << " person type " << type << std::endl;

            suc = getCommandInterface()->addPerson(per.str(), type, edge, simTime());
            if (!suc) {
                i->second.pop();
            }
            else {
                MYDEBUG << "successful inserted " << per.str() << std::endl;
                queuedPersons.insert(per.str());
                i->second.pop();
                personNameCounter++;
            }
        }
...
void TraCIScenarioManager::insertNewVehicle() {
    ...
    if (vehicleTypeIds.size()) {
        int vehTypeId = mobRngVeh->intRand(vehicleTypeIds.size());
        type = vehicleTypeIds[vehTypeId];
    }
    else {
        type = "DEFAULT_VEHTYPE";
    }
    int routeId = mobRngVeh->intRand(routeIds.size());
...
void TraCIScenarioManager::processSimSubscription(std::string objectId, TraCIBuffer& buf) {
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j) {
        ...
        if (variable1_resp == VAR_DEPARTED_VEHICLES_IDS) {
            ...
            if ((count > 0) && (count >= activeVehicleCount) && autoShutdown && noMoreExpectedVehicle) autoShutdownTriggered = true;
            activeVehicleCount -= count;
            drivingVehicleCount -= count;
            ...
            uint8_t varType; buf >> varType;
            ASSERT(varType == TYPE_INTEGER);
            uint32_t nExpectedVehicles; buf >> nExpectedVehicles;
            MYDEBUG << "TraCI reports " << nExpectedVehicles << " expected vehicle(s)." << endl;
            if (nExpectedVehicles == 0) {
                noMoreExpectedVehicle = true;
            }
...
void TraCIScenarioManager::processVehicleSubscription(std::string objectId, TraCIBuffer& buf) {
    ...
    if (!mod) {
        ...
        if (mType != "0") {
            addModule(VEHICLE_TYPE, objectId, mType, mName, mDisplayString, p, edge, speed, angle);
        ...
        for (cModule::SubmoduleIterator iter(mod); !iter.end(); iter++) {
            cModule* submod = SUBMODULE_ITERATOR_TO_MODULE(iter);
            ifInetTraCIMobilityCallNextPosition(VEHICLE_TYPE, submod, p, edge, speed, angle);
            TraCIVehicleMobility* mm = dynamic_cast<TraCIVehicleMobility*>(submod);
            if (!mm) continue;
...
//************* Quang-Huy Nguyen *******************
void TraCIScenarioManager::processPersonSubscription(std::string objectId, TraCIBuffer& buf, bool isInit_traci) {
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j) {
        ...
        if (isokay != RTYPE_OK) {
            ...
            std::string errormsg; buf >> errormsg;
            if (isSubscribed) {
                if (isokay == RTYPE_NOTIMPLEMENTED) error("TraCI server reported subscribing to person variable 0x%2x not implemented (\"%s\"). Might need newer version.", variable1_resp, errormsg.c_str());
                error("TraCI server reported error subscribing to person variable 0x%2x (\"%s\").", variable1_resp, errormsg.c_str());
            }
        } else if (variable1_resp == ID_COUNT) {
            uint8_t varType; buf >> varType;
            ASSERT(varType == TYPE_INTEGER);
            uint32_t nPersons; buf >> nPersons;
            MYDEBUG << "TraCI reports " << nPersons << " person(s) in total." << endl;
            if (nPersons == 0) {
                noMorePersons = true;
            }
        } else if (variable1_resp == ID_LIST) {
            ...
            ASSERT(varType == TYPE_STRINGLIST);
            uint32_t count; buf >> count;
            MYDEBUG << "TraCI reports " << count << " active persons." << endl;
            //            ASSERT(count == activePersonCount);
            ...
            std::set<std::string> needSubscribe;
            std::set_difference(activePersons.begin(), activePersons.end(), subscribedPersons.begin(), subscribedPersons.end(), std::inserter(needSubscribe, needSubscribe.begin()));
            for (std::set<std::string>::const_iterator i = needSubscribe.begin(); i != needSubscribe.end(); ++i) {
            ...
            std::set<std::string> needUnsubscribe;
            std::set_difference(subscribedPersons.begin(), subscribedPersons.end(), activePersons.begin(), activePersons.end(), std::inserter(needUnsubscribe, needUnsubscribe.begin()));
            for (std::set<std::string>::const_iterator i = needUnsubscribe.begin(); i != needUnsubscribe.end(); ++i) {
            ...
            error("Received unhandled person subscription result");
        }
    }
    ...
    if (!inRoi) {
        if (mod) {
            ...
            unEquippedHosts.erase(objectId);
            MYDEBUG << "Person (unequipped) # " << objectId << " left region of interest" << endl;
        }
        return;
    }
    ...
    if (!mod) {
        ...
        if (iType == moduleType.end()) {
            iType = moduleType.find("*");
            if (iType == moduleType.end())
                throw cRuntimeError("cannot find a module type for person type \"%s\"", pType.c_str());
        }
        mType = iType->second;
        // search for module name
        ...
        if (iName == moduleName.end()) {
            iName = moduleName.find(std::string("*"));
            if (iName == moduleName.end())
                throw cRuntimeError("cannot find a module name for person type \"%s\"", pType.c_str());
        }
        mName = iName->second;
        // search for module display string
        if (moduleDisplayString.size() != 0) {
            iDisplayString = moduleDisplayString.find(pType);
            if (iDisplayString == moduleDisplayString.end()) {
                iDisplayString = moduleDisplayString.find("*");
                if (iDisplayString == moduleDisplayString.end())
                    throw cRuntimeError("cannot find a module display string for person type \"%s\"", pType.c_str());
            }
            mDisplayString = iDisplayString->second;
        }
        else {
        ...
        if (mType != "") {
            addModule(PERSON_TYPE, objectId, mType, mName, mDisplayString, p, edge, speed, angle);
        ...
        for (cModule::SubmoduleIterator iter(mod); !iter.end(); iter++) {
            cModule* submod = SUBMODULE_ITERATOR_TO_MODULE(iter);
            ifInetTraCIMobilityCallNextPosition(PERSON_TYPE, submod, p, edge, speed, angle);
            TraCIPersonMobility* mm = dynamic_cast<TraCIPersonMobility*>(submod);
            if (!mm) continue;
            MYDEBUG << "module " << objectId << " moving to " << p.x << "," << p.y << endl;
            mm->nextPosition(p, edge, speed, angle);
...
void TraCIScenarioManager::processSubcriptionResult(TraCIBuffer& buf, bool isInit_traci) {
    uint8_t cmdLength_resp; buf >> cmdLength_resp;
    ...
    //************* Quang-Huy Nguyen *******************
    else if (commandId_resp == RESPONSE_SUBSCRIBE_PERSON_VARIABLE) processPersonSubscription(objectId_resp, buf, isInit_traci);
    //**************************************************
    else {
...
Veins::TraCIConnection* TraCIScenarioManager::getConnection(){
