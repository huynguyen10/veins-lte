...
std::list<std::string> TraCICommandInterface::getEdgeIds() {
...
//************* Quang-Huy Nguyen *******************
bool TraCICommandInterface::addPerson(std::string personId, std::string personTypeId, std::string edgeId, simtime_t emitTime_st, double emitPosition) {
    bool success = false;
    ...
    int32_t count = 4;
    int32_t emitTime = (emitTime_st < 0) ? (-1) : (floor(emitTime_st.dbl() * 1000));
    TraCIBuffer buf = connection.queryOptional(CMD_SET_PERSON_VARIABLE, TraCIBuffer() << variableId << personId << variableType << count << (uint8_t)TYPE_STRING << personTypeId << (uint8_t)TYPE_STRING << edgeId << (uint8_t)TYPE_DOUBLE << emitPosition << (uint8_t)TYPE_INTEGER << emitTime, success);
    ASSERT(buf.eof());
