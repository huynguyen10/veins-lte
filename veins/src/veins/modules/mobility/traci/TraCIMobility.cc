...
Coord TraCIMobility::calculateAntennaPosition(const Coord& pos) const {
	Coord corPos;
	if (antennaPositionOffset >= 0.001) {
		//calculate antenna position of vehicle according to antenna offset
		corPos = Coord(pos.x - antennaPositionOffset*cos(angle), pos.y + antennaPositionOffset*sin(angle), pos.z);
	} else {
		corPos = Coord(pos.x, pos.y, pos.z);
	}
	return corPos;
}
